...
**DiGi.User.PostgreSQL** is a C# library that persists the `DiGi.User` model to a PostgreSQL database. It mirrors the structure and converter architecture of **DiGi.GIS.PostgreSQL**, and is intended to be consumed by a `DiGi.User.WebAPI` (or similar) host the same way the GIS WebAPI consumes its converters.
...
* **[DiGi.User.PostgreSQL](DiGi.User.PostgreSQL)** (Path: `DiGi.User.PostgreSQL\DiGi.User.PostgreSQL`)

The assembly references its sibling libraries by `HintPath` (never `ProjectReference`):
* **DiGi.User** — the `User` model (email, first name, last name).
* **DiGi.PostgreSQL** — the `PostgreSQLConverter<TObject>` base, `ConnectionData`, `Create.NpgsqlConnection`, `Modify.ClearAsync`, and the `IPostgreSQLConverter` / `IPostgreSQLObject` contracts.
* **DiGi.Core** — `SerializableObject`, `IObject`, and the shared serialization pattern.

---

## 📐 Core Architectural Pattern (DiGi.Core Pattern)

This project strictly separates **Data Models** (anemic schemas) from **Database/Conversion Logic** (static extension methods and converter classes).
...
### 1. Data Models (Classes, Interfaces)
...
### 2. Database Logic
* **Converters (`/Classes/Converter/`):** Class-specific converters manage mapping between the DiGi model objects and PostgreSQL tables. `UserPostgreSQLConverter` converts `User` entities to and from the `users` table; `UserPostgreSQLConverterManager` is the registry.
* **Schema Creation (`/Create/TableAsync.cs`):** Static asynchronous methods responsible for DDL execution (table creation, index generation). All DDL commands are idempotent (`IF NOT EXISTS`).
* **Queries (`/Query/`):** Read-only data access.
* **Modifications (`/Modify/`):** Write/update/delete operations.

---

## 🔌 PostgreSQL Converter Registration

Converters in this project must be explicitly registered in `Create\UserPostgreSQLConverterManager.cs`. This factory method reads the `PostgreSQL_Main` configuration file and manually instantiates each converter.

The User WebAPI project's `InitializeAsync` reads converters from the returned `UserPostgreSQLConverterManager` (which derives from `DiGi.PostgreSQL.Classes.PostgreSQLConverterManager<IUserPostgreSQLConverter>`) and registers them in the DI container. WebAPI controllers depend on these converters via constructor injection.

**When adding a new converter that will be consumed by a User WebAPI controller:**
1. Add its instantiation to `Create\UserPostgreSQLConverterManager.cs` under the appropriate configuration file section.
...
A user is stored as a single row in the `users` table, addressed at **one level**:

| To address | Use | Meaning |
|---|---|---|
| One user | `email` | The user's unique email (the natural key) |
| Surrogate id | `id` | The `SERIAL` primary key (assigned by the database) |

* **`email` is the natural key and is `NOT NULL UNIQUE`.** Because it is non-nullable, `INSERT ... ON CONFLICT (email) DO UPDATE` is deterministic and needs no `NULLS NOT DISTINCT` (that rule only applies to composite keys that include nullable columns).
* **Writes are upserts by email:** storing a user refreshes the existing row rather than creating a duplicate, and `first_name`, `last_name`, and the `object` JSONB envelope are refreshed together.
* **`object` holds the full serialized `User`** so a future property added to the model can be recovered even before a matching column exists; the denormalized `first_name` / `last_name` columns exist so reads and filters never need to parse the envelope.

---

## 🗄️ Connection Assets

Connection strings and credentials are loaded from a `*.conf` file located in the git-ignored `user files/` directory (e.g. `user files/User_PostgreSQL_Main.conf`). **Never commit connection strings containing passwords, host IPs, or secret tokens to source control.** A `.conf` always points at a **development** database — it is the right place to exercise a code path, but it is not production.
